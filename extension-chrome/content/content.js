;(function(){
	...
	const viewerContentWindow = viewerMainWindow.querySelector("#image-shower-view-area");
	
	// html элемент 'img' - в него передается ссылка на изображение и размер изображения.
	const viewerImgElement = viewerMainWindow.querySelector("#image-shower-img-element");
	
	// контейнер - для контролов, обертка для элементов управления просмотром.
	const controllsConteiner = viewerMainWindow.querySelector("#image-shower-controls");
	
	const btnShowerFullscreen = viewerMainWindow.querySelector(".image-shower-controls-btn-fullscreen");

	const btnShowerClose = controllsConteiner.querySelector(".image-shower-controls-btn-close");

	const btnDownloadImage = viewerMainWindow.querySelector(".image-shower-controls-btn-download");

	btnShowerClose.style.backgroundImage = `url(${chrome.runtime
		.getURL("/content/img/icon-jris-btn-close.png")})`;

	btnDownloadImage.style.backgroundImage = `url(${chrome.runtime
		.getURL("/content/img/icon-jris-btn-download.png")})`;

	const urlIconOpenFullscreen = `url(${chrome.runtime
		.getURL("/content/img/icon-jris-btn-fullscreen-open.png")})`;

	const urlIconCloseFullscreen = `url(${chrome.runtime
		.getURL("/content/img/icon-jris-btn-fullscreen-close.png")})`;

	const urlIconLoaderIndication = `url(${chrome.runtime
		.getURL("/content/img/icon-jris-img-loader.gif")})`;
	...
	function adaptationImageSizeForViewer() {
		...
		if (viewerContentWindow.clientHeight > imgSize.h) {
			viewerImgElement.style.marginTop = `${ (viewerContentWindow.clientHeight - imgSize.h) / 2 }px`;
		} else {
	...
	function openViewer() {
		...
		let loaded = false;
		setTimeout(() => {
			if (!loaded) viewerContentWindow.style.backgroundImage = urlIconLoaderIndication;
		}, 15);
		sourceImage.onload = () => {
	...
	btnDownloadImage.addEventListener("click", (e) => {
		chrome.runtime.sendMessage(extensionId, { url: sourceImage.src }, (response) => {
	...
	document.getElementById("contentinner")?.addEventListener("click", function(event) {
		
		if (event.target.tagName === "IMG" 
		&& (event.target.closest(".image") || event.target.closest(".post_comment_list"))) {
